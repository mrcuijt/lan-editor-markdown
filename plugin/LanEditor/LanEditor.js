var LanEditor = {
    ...
    RenderHTML: function() {
        ...
        // console.info($("#" + this.showelem).html());
        // console.info(this.showelem);
        // 遍历 div#showelem 元素下的所有 pre code 对其进行高亮显示
        $('pre code', $("#" + this.showelem)).each(function(i, block) {
            //
            // 修改
            // 1、找到向上找到 pre 标签，使用 Syntax Highlighter 进行处理高亮显示
        ...
        try{
            if (typeof(eval(autosize)) == "function") {
                $('p>text').each(function (index, item, arry) {
                    var oTextarea = $("<textarea>").html($(item).html());
                    oTextarea.attr("zoom", $(item).attr("zoom"));
                    // console.info(oTextarea);
                    // console.info($(item).parent());
                    $(item).parent().html(oTextarea);
                });
                autosize($('p>textarea'));
                autosize($('div>textarea'));
                $('#show textarea').each(function (index, item, arry) {
                    var $item = $(item);
                    ...
                    if(zoom){
                        ...
                        var oarry = $(item).text().split("\n")

                        try{
                            oarry.forEach(function(item,index,arry){
                                if(item){
                                    $item.width(item.length * zoom + "px");
                                    throw "";
                                }
                            })
                        }catch (e){
    ...
    File: {
        ...
        ExportHTML: function(md) {
            var fileName = prompt("保存为HTML文件，请输入文件名", document.title);
            if (fileName == null || fileName == "") {
                return;
            }
